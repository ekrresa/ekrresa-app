...
export function MobileFabMenu({ path }: MobileFabMenuProps) {
  ...
  const itemDelays = ['delay-75', 'delay-100', 'delay-150']
  ...
  return (
    <div className="lg:hidden">
      ...
      >
        ...
        <span
          className={`absolute inset-0 rounded-full bg-accent/12 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] dark:bg-[#8fa0ea]/18 ${
      ...
      {isMenuMounted ? (
        <>
          <div
            className={`fixed inset-0 z-30 bg-[#0c1220]/30 backdrop-blur-[2px] transition-all duration-400 ease-out motion-reduce:transition-none ${
          ...
          <nav
            id="mobile-site-menu"
            aria-hidden={!isOpen}
            className={`fixed bottom-[calc(env(safe-area-inset-bottom,0)+5.5rem)] right-4 z-35 w-[min(calc(100vw-2rem),22rem)] origin-bottom-right rounded-4xl border border-black/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(250,244,238,0.98))] p-3 shadow-[0_28px_80px_rgba(24,21,17,0.2)] backdrop-blur-xl transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] motion-reduce:transition-none dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(23,30,47,0.98),rgba(13,18,30,0.98))] dark:shadow-[0_32px_84px_rgba(0,0,0,0.42)] ${
              isOpen
                ? 'translate-y-0 scale-100 opacity-100'
          ...
          >
            <div
              className={`mb-3 flex items-center justify-between gap-3 rounded-[1.4rem] border border-black/8 bg-white/55 px-4 py-3 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] dark:border-white/8 dark:bg-white/6 ${
            ...
            >
              <div>
                ...
                <p className="mt-1 text-sm font-medium text-ink dark:text-white">Menu</p>
              </div>
              <div className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-black/8 bg-white/72 text-accent dark:border-white/10 dark:bg-white/8 dark:text-[#d9c8f1]">
            ...
            <div className="space-y-2">
              {mobileMenuItems.map(item => {
                ...
                const itemDelay = itemDelays[mobileMenuItems.indexOf(item)] ?? 'delay-150'
                ...
                return (
                  <a
                    ...
                    className={`group flex items-center justify-between rounded-[1.4rem] border px-4 py-3.5 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                      active
                    ...
                    } ${isOpen ? `translate-y-0 opacity-100 ${itemDelay}` : 'translate-y-3 opacity-0'}`}
                    onClick={closeMenu}
                  >
            ...
            <div
              className={`mt-3 rounded-[1.4rem] border border-black/8 bg-white/55 px-4 py-3 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] dark:border-white/8 dark:bg-white/6 ${
